...
def _map_update(successes, trials, prior_a, prior_b, eps=1e-9):
    return (successes + prior_a - 1) / (trials + prior_a + prior_b - 2 + eps)
...
def EM_algorithm(
    ...
    R = np.zeros((n_solvers, n_instances), dtype=np.uint8)   # r_{s,i}
    C = np.zeros((n_solvers, n_instances), dtype=np.int32)   # C_{s,i}
    N = np.zeros((n_solvers, n_instances), dtype=np.int32)   # tests run

    for s, i, passed in df[["solver", "instance", "result"]].values:
        R[s, i] = 1
        C[s, i] += passed
        N[s, i] += 1 if n_tests == 1 else n_tests  # support aggregated counts
    ...
    for itr in range(max_iter):
        ...
        a0_, b0_ = p0 * (1 / rho0 - 1), (1 - p0) * (1 / rho0 - 1)
        a1_, b1_ = p1 * (1 / rho1 - 1), (1 - p1) * (1 / rho1 - 1)

        logL0 = _log_beta_binom(C, N, a0_, b0_)
        logL1 = _log_beta_binom(C, N, a1_, b1_)

        log_like1 = (1 - R) * np.log(beta_s + epsilon)[:, None]
        log_like0 = (1 - R) * np.log(1 - alpha_s + epsilon)[:, None]

        log_mix = np.logaddexp(
            np.log(gamma_s + epsilon)[:, None] + logL1,
            np.log(1 - gamma_s + epsilon)[:, None] + logL0,
        )

        log_like1 += R * (np.logp(1 - beta_s + epsilon)[:, None] + log_mix)
        log_like0 += R * (np.log(alpha_s + epsilon)[:, None] + logL0)

        logLambda1 = log_like1.sum(0)
        logLambda0 = log_like0.sum(0)

        log_fi_num = np.log(lambda_ + epsilon) + logLambda1
        log_fi_den = np.logaddexp(log_fi_num, np.log(1 - lambda_ + epsilon) + logLambda0)
        fi_expect = np.exp(log_fi_num - log_fi_den)

        fsi_expect = np.zeros_like(C, dtype=float)
        s_idx, i_idx = np.where(R)
        num = np.log(gamma_s[s_idx] + epsilon) + logL1[s_idx, i_idx]
        den = np.logaddexp(num, np.log(1 - gamma_s[s_idx] + epsilon) + logL0[s_idx, i_idx])
        fsi_expect[s_idx, i_idx] = fi_expect[i_idx] * np.exp(num - den)

        # M‑STEP
        lambda_ = _map_update(fi_expect.sum(), n_instances, *priors["lambda"])
        alpha_s = _map_update(((1 - fi_expect)[None, :] * R).sum(1), (1 - fi_expect).sum(), *priors["alpha_s"])
        beta_s  = _map_update((fi_expect[None, :] * (1 - R)).sum(1), fi_expect.sum(), *priors["beta_s"])
        gamma_s = _map_update((fsi_expect * R).sum(1), (fi_expect[None, :] * R).sum(1), *priors["gamma_s"])
        ...
        def _update_rho(weight):
            ...
            if not mask.any():
                return 1e-4
            w = weight[mask]
            n_bar = (w * N[mask]).sum() / w.sum()
            p_hat = (w * (C / N)[mask]).sum() / w.sum()
            var_hat = (w * ((C / N)[mask] - p_hat) ** 2).sum() / w.sum()
            rho = (n_bar * var_hat / (p_hat * (1 - p_hat) + tiny) - 1) / (n_bar - 1 + tiny)
            return np.clip(rho, 1e-4, 1 - 1e-4)

        rho1 = _update_rho(fsi_expect * R)
        rho0 = _update_rho((1 - fsi_expect) * R)

        # Convergence check
        if itr:
            delta = max(
                abs(np.log(lambda_) - np.log(prev_lambda)),
        ...
        prev_lambda, prev_alpha_s, prev_beta_s, prev_gamma_s = (
            lambda_, alpha_s.copy(), beta_s.copy(), gamma_s.copy()
        )
...
##### Robust WSCP #####
def evaluate_feasibility_robust(data, r):
    emitters = data['emitters']
    clients = data['clients']
    K = data['K']
    sel = r.get('selected_emitters', [])
    obj = r.get('objective_value', None)
    ...
    total_cost = sum(emitters[i]['cost'] for i in sel)
    if obj is None or total_cost != obj:
        return False

    for cx, cy in clients:
        cover_count = 0
        for i in sel:
            ex, ey = emitters[i]['position']
            r = emitters[i]['radius']
            if math.hypot(cx - ex, cy - ey) <= r + 1e-9:
                cover_count += 1
        if cover_count <= K:
            return False
